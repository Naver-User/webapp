...
// Getter/Setter 메소드에서, 접두사를 뺀 이름으로나오는 소위 "프로퍼티(Property)"를
// 통해서 필드의 데이터를 조작하게 됩니다.

// 읽기전용객체 / 영속성 계층에서 발생된 데이터를 보관하는 객체
// VO 패턴의 규칙
//
// (1) Serializable Tag 인터페이스를 구현하라! (선택)
// (2) 필드의 이름과 개수는 통산, 특정 테이블의 스키마와 동일하게 선언 
//		- 테이블의 컬럼이름을 자바식별자 규칙에 맞게 선언
//		- 필드의 타입은, 컬럼의 타입에 대응되는 자바타입으로 선언
//		- 테이블의 스키마대로 모든 컬럼을 필드로 매핑합니다.
// (3) 모든 필드는 기본으로 private 접근제한자 적용(은닉화=캡슐화)
// (4) 생성자는 모든 필드를 매개변수로 가지는 매개변수 있는 생성자만
//		오직 선언해야 합니다(왜? 읽기전용객체로 만들어야 하기 때문에)
// (5) 2단계 중복판정 알고리즘에 대비해서 구현되어야 된다!
//		(왜? JCF 자료구조에 저장되는게 일반적이니까.. 중복판정이 가능해야 하죠)
// (6) VO/DTO 모두, 필드의 타입으로 절대 기본타입을 사용해서는 안된다!!!
//	   참조타입을 써야 된다! (왜????)
public class EmpVOBeforeLombok implements Serializable { // POJO : Plain Old Java Object.
